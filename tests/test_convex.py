...
class TestPolygon:
    ...
    # Количество вершин, лежащих в окрестности заданного квадрата
    def test_count_sq1(self):
        assert self.f.count_sq() == 3

    def test_count_sq2(self):
        assert self.f.add(R2Point(4.0, 4.0)).count_sq() == 3

    def test_count_sq3(self):
        assert self.f.add(R2Point(3.5, 3.5)).count_sq() == 4

    def test_count_sq4(self):
        assert self.f.add(R2Point(
            0.0, 4.0)).add(R2Point(
                4.0, 0.0)).add(R2Point(-1.0, -1.0)).count_sq() == 2

    def test_count_sq5(self):
        assert self.f.add(R2Point(
            0.0, 4.1)).add(R2Point(
                4.1, 0.0)).add(R2Point(-1.0, -1.0)).count_sq() == 0

    def test_count_sq6(self):
        assert self.f.add(R2Point(
            2.0, 4.0)).add(R2Point(
